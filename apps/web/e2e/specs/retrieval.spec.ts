...
test.describe('检索可视化', () => {
  ...
  test('TC-RETR-02: 点击打开检索详情抽屉', async ({ page }) => {
    ...
    // 抽屉标题应出现
    await expect(page.getByRole('heading', { name: '检索详情' })).toBeVisible({ timeout: 3000 });
    // 检索来源贡献区域
    await expect(page.getByText('检索来源贡献')).toBeVisible();
    await expect(page.getByText('向量检索').first()).toBeVisible();
    await expect(page.getByText('关键词检索').first()).toBeVisible();
  });
