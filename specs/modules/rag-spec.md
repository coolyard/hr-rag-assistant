...
| `vector_weight`        | 0.4    | 混合检索中向量检索权重（默认值，mixed 类别；由 QueryClassifier 动态覆盖）         |
| `keyword_weight`       | 0.6    | 混合检索中关键词检索权重（默认值，mixed 类别；由 QueryClassifier 动态覆盖）       |
| `dynamic_weight`       | 自适应 | 根据查询分类（exact-keyword: 0.2+0.8, semantic: 0.7+0.3, mixed: 0.4+0.6）动态选择 |
...
### 3.2 关键词检索（BM25 + 同义词扩展）

**实现**：MiniSearch BM25 索引引擎（前缀匹配、模糊匹配 fuzz=0.2、字段权重 title:3/content:1.5/category:1）

**同义词扩展**：查询前自动追加同义词（如"医保"→"医疗保险"），提升召回覆盖。

**预定义关键词库**（45+ 个中文 HR 关键词）：
...
function keywordSearch(query: string, chunks: SearchResult[], topK: number): SearchResult[] {
  // 1. 从查询中提取匹配关键词
...
function shouldReject(results: MergedResult[], query: string): boolean {
  ...
  // 条件 4: 涉及公司机密
  const secretPatterns = [/裁员/, /收购|并购/, /季度财报.*未公布/];
...
async function buildPrompt(
  ...
  // 3. 组装 Prompt
  const systemPrompt = SYSTEM_PROMPT_TEMPLATE.replace('{{retrieved_chunks}}', formatChunks(chunks))
...
```typescript
class RAGService {
  async orchestrate(query: string, conversationId: string): Promise<AsyncIterable<StreamChunk>> {
    // Step 1: 检索
    ...
    // Step 4: 组装 Prompt
    const { formattedHistory, formattedChunks } = this.buildPrompt(merged, history, query);
...
- [ ] 混合检索权重正确：查询自适应动态权重（exact-keyword: 0.2+0.8, semantic: 0.7+0.3, mixed: 0.4+0.6）
- [ ] 多轮对话能正确承接上下文
...
| 2026-07-07 | v1.1 | 关键词检索升级为 MiniSearch BM25 + 同义词扩展；新增 QueryClassifier 动态权重；修正"RRF 简化版"为"加权分数合并" |
