...
@Module({
  imports: [AskModule, AuthModule, ChatModule, DocumentModule, 
    EmbeddingModule, HealthModule, LLMModule, RagModule, 
    UserProfileModule, VectorModule],
...
编译后变为：
```css
.Navbar_navbar__3x7a9 { background: var(--bg-primary); }
```
...
function mergeResults(vectorResults, keywordResults, topK) {
  ...
  return Array.from(merged.values()).sort((a, b) => b.hybridScore - a.hybridScore).slice(0, topK);
}
```
...
if (existsSync(webDistPath)) {
  ...
  expressApp.use((req, res, next) => {
    if (req.path.startsWith('/api/')) { next(); return; }  // API 路由穿透
    if (req.path.includes('.')) { next(); return; }        // 静态资源穿透
